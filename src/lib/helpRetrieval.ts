...
const FORCE_HEADING_JA: Array<{ re: RegExp; keys: string[] }> = [
  ...
  {
    re: /セクション.*追加|パート.*追加|add section/i,
    keys: ["8.", "セクション", "追加"],
  },
  ...
  {
    re: /メガファイル|ギガファイル|ファイル便|データ便|mega\.nz|megaファイル|ドライブ|dropbox|mp3|直リンク|使える.*url|url.*使える/i,
    keys: ["13.", "音源", "使え", "対応"],
  },
  { re: /動画|youtube|参考動画/i, keys: ["13.", "参考動画"] },
  { re: /ログイン|パスワード|アカウント/i, keys: ["3.", "ログイン"] },
  {
    re: /名前|ネーム|改名|リネーム|名称/i,
    keys: [
      "4.",
      "5.",
      "7.",
...
const FORCE_HEADING_EN: Array<{ re: RegExp; keys: string[] }> = [
  ...
  {
    re: /mega|file.?bin|file transfer|google drive|dropbox|mp3|direct link|can i use.*url/i,
    keys: ["13.", "Not supported", "Music"],
  },
  { re: /login|password|account/i, keys: ["3.", "Login"] },
  {
    re: /name|rename|title/i,
    keys: [
      "4.",
      "5.",
      "7.",
